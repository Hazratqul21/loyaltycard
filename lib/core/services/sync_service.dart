...
class SyncService {
  ...
  Future<SyncResult> syncAll() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('✅ Sinxronizatsiya tugadi: $uploaded yuklandi, $downloaded yuklab olindi');
      }
  ...
  Future<int> _uploadPendingChanges(String userId) async {
    ...
    if (pendingCards.isNotEmpty) {
      // userId ni qo'shish
      final cardsWithUserId = pendingCards.map((c) => LoyaltyCardModel(
        id: c.id,
    ...
    if (pendingTransactions.isNotEmpty) {
      final txWithUserId = pendingTransactions.map((t) => TransactionModel(
        id: t.id,
        cardId: t.cardId,
    ...
    if (pendingRewards.isNotEmpty) {
      final rewardsWithUserId = pendingRewards.map((r) => RewardModel(
        id: r.id,
        title: r.title,
  ...
  Future<int> _downloadServerChanges(String userId) async {
    ...
    final serverTransactions = serverData['transactions'] as List<TransactionModel>;
    ...
    for (final serverCard in serverCards) {
      final localCard = _localDatasource.getCard(serverCard.id);
      
      // Agar lokalda yo'q bo'lsa yoki serverniki yangiroq bo'lsa
      if (localCard == null || 
          serverCard.lastModifiedAt.isAfter(localCard.lastModifiedAt)) {
        await _localDatasource.updateCard(serverCard.copyWith(syncStatusString: 'synced'));
        count++;
      }
    }
    ...
    for (final serverTx in serverTransactions) {
      ...
      if (localTx == null || 
        ...
        await _localDatasource.updateTransaction(serverTx.copyWith(syncStatusString: 'synced'));
        count++;
      }
    }
    ...
    for (final serverReward in serverRewards) {
      ...
      if (localReward == null || 
        ...
        await _localDatasource.updateReward(serverReward.copyWith(syncStatusString: 'synced'));
        count++;
      }
    }
  ...
  Future<SyncResult> backup() async {
    ...
    try {
      ...
      final cardsWithUserId = cards.map((c) => LoyaltyCardModel(
        id: c.id,
        storeName: c.storeName,
        storeLogoUrl: c.storeLogoUrl,
        currentPoints: c.currentPoints,
      ...
      final txWithUserId = transactions.map((t) => TransactionModel(
        id: t.id,
        cardId: t.cardId,
        storeName: t.storeName,
        amount: t.amount,
      ...
      final rewardsWithUserId = rewards.map((r) => RewardModel(
        id: r.id,
        title: r.title,
        description: r.description,
        requiredPoints: r.requiredPoints,
